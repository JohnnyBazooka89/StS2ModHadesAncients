...
// adjusted based on https://github.com/Blizzarre/Runesmith2-StS2/blob/beta/Runesmith2Code/Data/RunMetricsUploader.cs

public static class HadesAncientsMetrics
{
    private static readonly RunMetricsUploader<RunMetrics> Uploader = new(
        new MetricsUploaderConfig
        {
            ModName = "Hades Ancients",
            EndpointUrl = "http://hadesancients.atwebpages.com/",
            ModVersionProvider = GetVersion,
            Logger = HadesAncientsMainFile.Logger,
            WrapPayload = (data, version, modName, hasForeign) =>
                $"{{\"hades_ancients_version\":{JsonSerializer.Serialize(version)},\"has_foreign_content\":{JsonSerializer.Serialize(hasForeign)},\"data\":{data}}}"
        },
        GetRunMetrics,
    ...
    {
        var mod = ModManager.GetLoadedMods().FirstOrDefault(m => m.manifest?.id == "HadesAncients");
